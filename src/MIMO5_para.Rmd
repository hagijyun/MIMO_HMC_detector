...
SAVED_TRIAL		<- TRUE																																		# ●Hやwを完全に再現できるため、提案法の繰り返し復号や方式別の個別復号が可能、読み込みメモリサイズを考慮してある程度小分、並列実行★
...
#HS_sigma <- c(2.0, 5.0, 3.0)						# ●提案法の超パラメータ馬蹄尤度を適用する際のσ_wに対する係数		TURBO符号の場合★
...
stan_mod_regular_turbo	<- stan_model(file = "MCMC4m.stan")					# 混合t分布+人為正則化、λがコーシー分布（尤度が馬蹄分布）
stan_mod_regular_no			<- stan_model(file = "MCMC4m3.stan")				# 混合t分布+人為正則化、λが未考慮
stan_mod_regular_uni		<- stan_model(file = "MCMC4m2.stan")				# 混合t分布+人為正則化、λが一様分布
...
## 送信・伝送路・受信・判定の実行
#	res <- foreach(ct = 1:length(SNR_LIST), .packages = c("rstan", "mvtnorm", "gtools", "foreach", "SD")) %dopar% {														# lapply(SNR_LIST, function(snr){	# SNR、並列時は%dopar%、直列時は%do%	
...
#			regular_lambda2 <- 0																																																									# ●正則化：なし（素のHMC）、コメントアウトでどちらか一つ★
		...
		### MMSEの結果を他方式の初期値として利用（未活用）
		MMSE_mu <- real.vec(u_hat_ID_MMSE)

		### 軟判定（MMSE-SIC）
		u_hat_MMSE_SIC <- MMSE_SIC(trial = trial)$u_hat_MMSE_SIC
		### 判定（MMSE-SIC）
		u_hat_ID_MMSE_SIC <- 								comp.vec(t(u_hat_MMSE_SIC))																																					# 軟値
...
######
if (TRUE){																																																																	# ●HMC、どちらか一つ★
	if (EXTRINSIC_LLR == FALSE){																																																							# ●誤り訂正：初発時
		### 軟判定（HMC、正則化+混合t分布）：HSなし, warm=12*2, 統計量は平均値
		fit_stan_q <- purrr::quietly(sampling)(object = stan_mod_regular,
												 data = list(n = n*2, m = m*2, y = as.vector(trial$y), H = trial$H, sigma_w2 = trial$sigma.w2, sigma_w = rep(sqrt(trial$sigma.w2), m*2), COMPONENT_NUM = COMPONENT_NUM, symb_real = QPSK_SYMB_real, BREAKS = BREAKS, sigma = MCMC_FIXED_SIGMA()*OPTIMAL_t_SIGMA_WEIGHT(), nu = OPTIMAL_nu(), regular_mu = opt$HMC$regular_mu, regular_OMEGA = opt$HMC$regular_OMEGA, log_q = LLR[ , , k, ct]),
												 chains = MR_MAX * round(MCMC_it_MAX/(n*2)), iter = 				n*2, warmup = 12 *2,
										 		 seed = 123)
		fit_stan_l <- fit_stan_q$result

		# 軟判定シンボルを出力
		u_hat_MCMC_l_chains <- extract(fit_stan_l, permuted = TRUE, inc_warmup = FALSE, par = c("u"))
		u_hat_MCMC_l <- colMeans(u_hat_MCMC_l_chains$u)																																													# 平均値
		### 判定（MCMC）
		u_hat_ID_turbo <- comp.vec(t(u_hat_MCMC_l))																																															# 軟値
	}else{																																																																		# 誤り訂正：繰返時
		### 軟判定（HMC、正則化+混合t分布）：HSあり, warm=12*2, 統計量はML値
		fit_stan_q <- purrr::quietly(sampling)(object = stan_mod_regular_turbo,																																	# λがコーシー分布★（択一）
#		fit_stan_q <- purrr::quietly(sampling)(object = stan_mod_regular_no,																																		# λが　　　未考慮★（択一）
#		fit_stan_q <- purrr::quietly(sampling)(object = stan_mod_regular_uni,																																		# λが　　一様分布★（択一）
												 data = list(n = n*2, m = m*2, y = as.vector(trial$y), H = trial$H, sigma_w2 = trial$sigma.w2, sigma_w = rep(sqrt(trial$sigma.w2), m*2), COMPONENT_NUM = COMPONENT_NUM, symb_real = QPSK_SYMB_real, BREAKS = BREAKS, sigma = MCMC_FIXED_SIGMA()*OPTIMAL_t_SIGMA_WEIGHT(), nu = OPTIMAL_nu(), regular_mu = opt$HMC$regular_mu, regular_OMEGA = opt$HMC$regular_OMEGA, log_q = LLR[ , , k, ct], MMSE_mu = MMSE_mu, MMSE_sigma = 0.2, HS_sigma = HS_sigma[COMPONENT_BITS]),
												 chains = MR_MAX * round(MCMC_it_MAX/(n*2)), iter = 				n*2, warmup = 12 *2,
										 		 seed = 123)
		fit_stan_l <- fit_stan_q$result
	...
	# デバグ用にStanの結果を保存する場合
	if(DEBUG_OUT){
		save(fit_stan_l, file = sprintf("e:/DEBUG_fit_stan%s%s_snr%02d_trial%04d_%s.RData", args[1], args[2], SNR_LIST[ct], k, format(Sys.time(), "%Y%m%d%H%M%S")))
	}
}else{
	u_hat_ID_turbo	<- NULL
}
...
if (FALSE){
		### 軟判定（SD）
		u_hat_SD <- SD(trial = trial)$u_hat_SD
		### 硬判定（SD）
		u_hat_ID_SD <- hard.descision(comp.vec(t(u_hat_SD)))
}else{
	u_hat_ID_SD <- NULL
}
...
# 軟判定値の抽出を行う関数
u_hat_complex_list <- function(method){																																	# 手法毎（NULLの可能性のあるものには、あらかじめ全0を付加）
	lapply(res, function(by_snr){																																					# SNR毎
		u_hat_complex_matrix <- sapply(by_snr, function(by_trial){																					# 試行回数毎
					 if (method == "MGS"			){ u_hat_complex <- as.vector(by_trial$u_hat_ID_MGS									+ rep(0, n)			)	}
			else if (method == "EP"				){ u_hat_complex <- as.vector(by_trial$u_hat_ID_EP																	)	}
			else if (method == "MMSE"			){ u_hat_complex <- as.vector(by_trial$u_hat_ID_MMSE																)	}
			else if (method == "MHGD"			){ u_hat_complex <- as.vector(by_trial$u_hat_ID_MHGD																)	}
			else if (method == "Lang"			){ u_hat_complex <- as.vector(by_trial$u_hat_ID_Lang																)	}
# ●コメントアウトでどちらか一つ★
#			else if (method == "HMC"			){ u_hat_complex <- as.vector(by_trial$u_hat_ID_MCMC_fixed_sigma		+ rep(0, n)			)	}
			else if (method == "HMC"			){ u_hat_complex <- as.vector(by_trial$u_hat_ID_turbo								+ rep(0, n)			)	}
			return(u_hat_complex)
		})
	})
}

# 軟判定値をファイルに書き出す関数
write_matlab <- function(method, artifitial_sigma = 0){
	tmp <- u_hat_complex_list(method = method)
	writeMat(con = sprintf("e:/%s%s.mat", method, args[1]), receivedSignal = array(unlist(tmp), dim = c(nrow(tmp[[1]]), ncol(tmp[[1]]), length(tmp))))
}

# 手法毎に軟判定値を保存
	write_matlab(method = "HMC")
if (EXTRINSIC_LLR == FALSE){
	write_matlab(method = "EP")
	write_matlab(method = "MMSE")
	write_matlab(method = "MGS")
#	write_matlab(method = "MHGD")
#	write_matlab(method = "Lang")
}
