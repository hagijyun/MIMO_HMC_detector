...
if (SAVED_SYMBOL == TRUE){
	MATLAB_symbols <- scan(file = "datasymbols.R", sep = ",")													# MATLAB出力を読み込み
...
### ●保存済みtrialを読み込む場合
if (SAVED_TRIAL == TRUE){
			 if (COMPONENT_BITS == 1){ trial_file <- sprintf("trail_%s%s.RData", "QPSK" , args[1]) }
	else if (COMPONENT_BITS == 2){ trial_file <- sprintf("trail_%s%s.RData", "16QAM", args[1]) }
	else if (COMPONENT_BITS == 3){ trial_file <- sprintf("trail_%s%s.RData", "64QAM", args[1]) }
	load(file = trial_file)
}

### ●シンボルLLRの設定
if (EXTRINSIC_LLR == TRUE){
	LLR <- readMat(sprintf("LLR%s.mat", args[1]))[[1]]
	source("LLR.R")
}else{
...
write_matlab <- function(method, artifitial_sigma = 0){
	...
	writeMat(con = sprintf("%s%s.mat", method, args[1]), receivedSignal = array(unlist(tmp), dim = c(nrow(tmp[[1]]), ncol(tmp[[1]]), length(tmp))))
}
